// IGXLRegistryHandler.cpp : Defines the exported functions for the DLL application.
//

#include "stdafx.h"
#include "IGXLRegistryHandler.h"
...
VOID DetAttach(PVOID *ppbReal, PVOID pbMine, PCHAR psz)
{
    ...
    if (l != 0) {
        REG_LOG("Attach failed: `%s': error %d\n", DetRealName(psz), l);
    }
}
...
VOID DetDetach(PVOID *ppbReal, PVOID pbMine, PCHAR psz)
{
    ...
    if (l != 0) {
        REG_LOG("Detach failed: `%s': error %d\n", DetRealName(psz), l);
    }
}
...
{
    REG_LOG("ASSERT(%s) failed in %s, line %d.\n", pszMsg, pszFile, nLine);
}
...
BOOL ProcessAttach(HMODULE hDll)
{
    ...
    if (error != NO_ERROR) {
        REG_LOG("Error attaching detours: %d\n", error);
    }
...
BOOL ProcessDetach(HMODULE hDll)
{
    ...
    if (error != NO_ERROR) {
        REG_LOG("Error detaching detours: %d\n", error);
    }
...
void InitializeRegistryDetour()
{
    _PrintEnter("InitializeRegistryDetour Enter");
    _PrintExit("InitializeRegistryDetour Exit");
}